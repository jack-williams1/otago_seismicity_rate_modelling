...
%% Plot NZ NSHM 2022 DSM forecasts with RSQSim and Stochastic Event Catalog
...
for kk=1:2
    ...
    if kk==1 %distributions based on annual rate of M>5 events
        ...
        for ff=1:2 %for each DSM forecast
    ...
    plot([x1_5_95(1),x1_5_95(1)],[0 1],'LineWidth',1,'Color','k'); hold on
    plot([x1_5_95(2),x1_5_95(2)],[0 1],'LineWidth',1,'Color','k'); hold on
    plot([x2_5_95(1),x2_5_95(1)],[0 1],'LineWidth',1,'Color',[0.5 0.5 0.5 1]); hold on
    plot([x2_5_95(2),x2_5_95(2)],[0 1],'LineWidth',1,'Color',[0.5 0.5 0.5 1]); hold on
    ...
    if kk==1
        legend([p1(1),p1(2),p1(3),p2,p3],{['neg-binom: ',num2str(urz_mag_rate(mag_indx1,1)*50,2)],['Poisson: ',num2str(urz_mag_rate(mag_indx1,2)*50,2)],...
            ['Hybrid: ',num2str(urz_mag_rate(mag_indx1,3)*50,1)],['RSQSim: ',num2str(mean(sampleAnnualRate_rsqsim_50(:,mag_indx2))*50,2)],...
           ['Stochastic: ',num2str(mean(sampleAnnualRate(:,mag_indx2))*50,2)]},'FontSize',fntsize,'Location',loc_opt(kk));

    else
              legend([p1(1),p1(2),p1(3),p2,p3],{['neg-binom:',newline,num2str(urz_mo_rate(1),2),' Nm/yr'],['Poisson:',newline,num2str(urz_mo_rate(2),2),' Nm/yr'],...
            ['Hybrid:',newline,num2str(urz_mo_rate(3),2),' Nm/yr'],['RSQSim:',newline,num2str(mean(sampleMoRate_rsqsim_50),3),' Nm/yr'],...
...
legend([p1(1), p1(2), p1(3), p2],{['URZ-negbinom, rate test: ', num2str(rate_count(1),3),'%'],...
...
ylabel('Annual frequency of exceedance'); xlabel('Magnitude');xlim([5 8]); ylim([5*10^-5 2]);
set(gca,'fontsize',fntsize+1);
    legend([p1(1), p1(2), p1(3), p3],{['URZ-negbinom, rate test: ', num2str(rate_count(1),2),'%'],...
        ['URZ-poi, rate test: ',num2str(rate_count(2),2),'%'],...
        ['hybrid, rate test: ',num2str(rate_count(3),2),'%'],['Stochastic event catalog']},'location','northeast','fontsize',fntsize-1)
axis square; grid on
